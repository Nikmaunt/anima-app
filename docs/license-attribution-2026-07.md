...
Инвариант, который он держит, — юридический, а не косметический: **всё,
что реально едет пользователю, должно быть покрыто публикуемой
атрибуцией.** Надмножество допустимо, пропуск — нет.

**Как он НЕ подключён, и почему — это важнее самого таска.** Первая
редакция объявляла `dependsOn("generateLibraryDefinitionsRelease")` и
`inputs.file` на генерируемом файле. Это уронило весь зелёный контур:

```
Some problems were found with the configuration of task
':app:generateLibraryDefinitionsRelease' (type 'AboutLibrariesTask').
  Reason: Task ':app:packageReleaseResources' uses this output of task
  ':app:generateLibraryDefinitionsRelease' without declaring an explicit
  or implicit dependency.
```

То есть у самого плагина aboutlibraries **не объявлена связь** между
генератором и `packageReleaseResources`, и стоит втянуть генератор в один
граф с `bundleRelease`, как Gradle роняет сборку на этом чужом дефекте.
Наша проверка наследовала проблему, которой не создавала.

Поэтому таск намеренно **ничего не тянет в граф**: он читает то, что уже
произвёл release-сборкой, `outputs.upToDateWhen { false }`, и внятно
падает с инструкцией, если файла нет. В CI он стоит **после** шага сборки
бандла (`ci.yml`), и порядок закреплён комментарием.

Цена этого решения, названная явно: корректность держится на **порядке
шагов CI, а не на графе задач**. Локально, запущенный до release-сборки,
таск отработает на устаревшем сгенерированном файле и промолчит. В CI
такого не бывает — бандл собирается в том же job раньше.
