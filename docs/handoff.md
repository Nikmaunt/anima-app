...
6. **`adb` на этой машине воскресает сам, и добить его не удалось.**
   Честный, неприятный итог v0.9, зафиксированный как есть.
   Третий DoD-сабагент нашёл живой `adb` там, где его уже дважды убивали,
   и доказал, что не он его породил (процесс старше его первой команды на
   90 с, PowerShell-профилей нет). Наблюдение прямым замером:

   | Время | Событие |
   |---|---|
   | 16:23:54 | убит adb → **новый** adb через 4 с |
   | 16:25:03 | `gradlew --stop` + убиты все `java` + убит adb → `Get-Process` вернул **NONE** |
   | 16:25:07 | **опять живой adb**, спустя 4 с, при мёртвом демоне Gradle |

   Первая гипотеза — «виноват переживший сборку демон Gradle, AGP держит
   в нём ADB-мост» — была правдоподобной (демон от 16:04 действительно
   жил в 16:23) и **не подтвердилась**: после его смерти adb всё равно
   вернулся. Родитель каждого нового adb — короткоживущий adb-клиент,
   которого на момент замера уже нет, так что кто именно его зовёт,
   изнутри сессии не установлено.

   Что из этого следует для следующего прогона:
   - **qemu/emulator убиваются насмерть и остаются мёртвыми** — дорогой
     сценарий (девятичасовой зомби v0.8) правилом закрыт;
   - **`adb` — нет**, и требовать в DoD «ноль процессов adb» бессмысленно,
     пока причина не найдена. Формулировать критерий как «нет живых
     qemu/emulator», а adb проверять отдельно и не считать красным сам
     факт его наличия;
   - причину искать снаружи сессии: кандидатов «Android Studio» и «демон
     Gradle» проверка **исключила** (studio/idea не запущены, демон убит).
