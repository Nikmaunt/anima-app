...
## АДДЕНДУМ (аудит v0.8, 2026-07-25) — исправление классификации

История выше НЕ переписана; это дополнение к ней.

**Число «197 open source libraries» в строке CONFIRMED (§CONFIRMED,
строка «Экран OSS-лицензий») классифицировано ОШИБОЧНО.**

Факт, снятый инструментом в прогоне v0.8: закоммиченный
`app/src/main/res/raw/aboutlibraries.json` содержал **222** элемента
массива `libraries` на обоих коммитах v0.6 (`a191442`, `b213528`) и
**227** на коммитах v0.7 (`5c35e9e`, `87ce862`). Парсер
`LicensesScreen.kt:105-121` применяет `.map` без единого фильтра и
выводит `state.libraries.size`, поэтому из 222-элементного файла экран
не может показать 197.

Что в той строке было проверено верно и остаётся CONFIRMED: экран
существует, маршрут заведён, список рендерится живьём.

Что классифицировано неверно: **само число 197** — оно не было сверено с
артефактом и должно было получить статус **UNVERIFIED**. Наиболее
вероятная причина расхождения (гипотеза, инструментом не подтверждённая):
на эмуляторе стоял APK более раннего коммита, чем проверяемое дерево.

Урок прогона, внесённый в процесс: число, увиденное глазами на
устройстве, — это наблюдение, а не подтверждение; в CONFIRMED оно
попадает только после сверки с артефактом, который его порождает.
Подробности — `docs/audit-v07.md` §0.1.
...
- GMD-сьют `atd34DebugAndroidTest`: **ЖЁЛТЫЙ, диагностирован.**
  ...
  mon-серии: hatch → Home → чат E2E; ЭТО было на коде v0.6). Дальнейшая
  диагностика v0.7 (полная хроника — в финальном отчёте): (а) на ATD
  этой машины экран эмулятора остаётся 100% чёрным всю жизнь теста
  (замерено покадрово: все пиксели (0,0,0)) — кадры не рендерятся
  вовсе, пересоздание GMD-девайса с нуля не помогает; средовой дефект
  ATD-на-этой-машине, НЕ регрессия приложения (тот же APK на
  google_apis рендерится и проходит онбординг); (б) в помпу виртуальных
  кадров добавлен 2ms yield — без него на медленных образах помпа
  душит main looper; (в) yield вскрыл детерминированную IME-гонку шага
  импорта души (карточка уезжает из композиции LazyColumn при открытой
  клавиатуре) — шаг починен (closeSoftKeyboard + swipe-ожидание) и
  доказанно проходится; (г) хвост сценария (после экспорта) на этой
  машине ещё зависает — открытый дефект тест-инфры, в бэклог v0.8 с
  полной трассой диагностики. Остальные 4 GMD-сьюта устройства
  (EncryptedUpgrade, MindBakeoff, WidgetSnapshot ×2) — зелёные.
