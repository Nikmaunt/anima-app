...
- Слабые: **`SPROUT`** — генерическая ромашка на коричневом эллипсе,
  ...
  ~25% кадра, силуэт читается как лист/щит, бледно-жёлтое на почти белом
  фоне (низкий контраст); **`MOTH`** — коричневое тело, мелкий масштаб.
...
5. **Контактный лист 8×4** частично уже существует как 32 голдена; но
   светлой/тёмной темы в них нет: `StillRender` принимает `night`, а не
   тему, и фон рисует сам рендерер. «8×4 × 2 темы» потребует расширения
   контракта отрисовки — аддитивно (параметр с дефолтом).
