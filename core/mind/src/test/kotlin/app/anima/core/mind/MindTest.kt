...
class MindTest {
    ...
    @Test
    fun `fake streams word chunks then done with full text`() = runTest {
    ...
    @Test
    fun `fake handler sees call index for retry scripting`() = runTest {
    ...
    fun `fact json parses clean output`() {
        val parsed = FactJson.parseCandidates(
    ...
    fun `fact json survives prose-wrapped and dirty output`() {
        val parsed = FactJson.parseCandidates(
            "Sure! Here you go:\n[{\"category\":\"unknown_future\",\"text\":\"a fact\"}, {\"text\":\"\"}]\nHope this helps!",
        )
