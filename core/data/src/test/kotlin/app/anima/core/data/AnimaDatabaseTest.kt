...
class AnimaDatabaseTest {
    ...
    @Test
    fun `supersede keeps old row but hides it from live`() =
        runTest {
    ...
    @Test
    fun `double supersede is a no-op - no marker overwrite, no orphan`() =
        runTest {
            val dao = db.soulFactDao()
    ...
    @Test
    fun `forget is a tombstone not a delete`() =
        runTest {
            val dao = db.soulFactDao()
            dao.insert(fact("fact-1"))
    ...
    @Test
    fun `notif repost with same id dedups`() =
        runTest {
    ...
    @Test
    fun `notif prune erases old events only`() =
        runTest {
            val dao = db.notifEventDao()
    ...
    @Test
    fun `unknown enum wire degrades safely through repository mapping`() =
        runTest {
            val dao = db.soulFactDao()
            dao.insert(fact("fact-1").copy(category = "category_from_the_future"))
            val repo =
                app.anima.core.data.repo
                    .SoulRepository(dao)
    ...
    @Test
    fun `meta put get roundtrip`() =
        runTest {
            val dao = db.metaDao()
            dao.put(
                app.anima.core.data.entity
                    .MetaEntity("creature_name", "Люмик"),
            )
            dao.put(
                app.anima.core.data.entity
                    .MetaEntity("creature_name", "Мия"),
            )
