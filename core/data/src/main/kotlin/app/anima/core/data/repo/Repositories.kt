...
class SoulRepository @Inject constructor(private val dao: SoulFactDao) {
    ...
    private fun SoulFact.toEntity() = SoulFactEntity(
        id = id,
...
class ChatRepository @Inject constructor(private val dao: ChatDao) {
    ...
    suspend fun append(role: ChatRole, text: String, atMillis: Long): ChatMessage {
...
@Singleton
class JournalRepository @Inject constructor(private val dao: BodyJournalDao) {
    ...
    suspend fun record(kind: JournalKind, atMillis: Long, detail: String? = null) {
    ...
    suspend fun countOfSince(kind: JournalKind, sinceMillis: Long): Int =
        dao.countOfKindSince(kind.wire, sinceMillis)
}

@Singleton
class NotifEventsRepository @Inject constructor(private val dao: NotifEventDao) {
    ...
    suspend fun eventsToday(startOfDayMillis: Long): List<app.anima.core.model.NotifEvent> =
        dao.since(startOfDayMillis).map {
            app.anima.core.model.NotifEvent(it.id, it.packageName, it.postedAtMillis, it.title, it.text)
        }
    ...
    suspend fun insert(id: String, packageName: String, postedAtMillis: Long, title: String, text: String?) {
...
class IdentityRepository @Inject constructor(
    ...
    suspend fun hatch(name: String, concept: CreatureConcept, seed: Long, nowMillis: Long) {
